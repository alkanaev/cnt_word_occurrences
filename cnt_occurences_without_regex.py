...
def count_occurrences_in_text(word, text) -> int:
    ...
    exclude: set = {"^","]",'"',"*","/","\\","?",".","{","=","<","%",")","#",
        "+","&","(","!","$","[","}",";",">","_","`","~","|",",",":","@","”","“","«","»"}
    list_words: str = "".join(i for i in text.replace(":", " ") if i not in exclude).split()
    pieces_variants: list = [i.lower() for i in list_words] + [i.lower().replace("'", "") for i in text.split()]
    text_pieces: list = []
    for piece in list_words:
        with contextlib.suppress(IndexError):
            upper = [i for i, e in enumerate(piece) if e.isupper()]
            if upper and not piece.isupper():
                for j in range(len(upper)):
                    try:
                        text_pieces.append(piece[upper[j] : upper[j + 1]])
                    except IndexError:
                        text_pieces.append(piece[upper[j] :])
                if upper[0] != 0:
                    text_pieces.append(piece[: upper[0]])
            else:
                text_pieces.append(piece)
    occurrences: dict = dict(Counter([i.lower() for i in text_pieces]))
    for k in list(occurrences):
        new_k = "".join(i for i in k if i.isalnum())
        occurrences[new_k] = occurrences[k]
    if word in text.lower():
        if len(word.split()) == 1:
            return occurrences[word] if word in occurrences else 0
        if word.split()[0] in pieces_variants and word.split()[-1] in pieces_variants:
            return len(text.lower().split(word.lower())) - 1
    return 0
...
# print(count_occurrences_in_text("I", text))
...
and my name is GEORGES"""
        ...
        self.assertEqual(1, count_occurrences_in_text("reflexion mirror", 'Reflexion Mirror" in Sopchoppy, Florida'))
        self.assertEqual(
            1,
            count_occurrences_in_text(
                "reflexion mirror",
        ...
        self.assertEqual(
            1,
            count_occurrences_in_text(
                "get back to me", "I hope you will consider this proupperal, and get back to me as soon as uppersible"
            ),
        )
        self.assertEqual(
            1,
        ...
        self.assertEqual(1, count_occurrences_in_text("get back to me", "When you know:get back to me"))
        self.assertEqual(
            1,
            count_occurrences_in_text(
                "don't be left",
...
Don't be left unprotected. Order your don SSSS3000 today!""",
        ...
        self.assertEqual(1, count_occurrences_in_text("take that as a 'yes'", "Do I have to take that as a 'yes'?"))
        self.assertEqual(1, count_occurrences_in_text("don't take that as a 'yes'", "I don't take that as a 'yes'?"))
        self.assertEqual(1, count_occurrences_in_text("take that as a 'yes'", "I don't take that as a 'yes'?"))
        self.assertEqual(1, count_occurrences_in_text("don't", "I don't take that as a 'yes'?"))
        self.assertEqual(
            1, count_occurrences_in_text("attaching my c.v. to this e-mail", "I am attaching my c.v. to this e-mail.")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Linguist", "'''Linguist Specialist Found Dead on Laboratory Floor'''")
        )
        self.assertEqual(
            1,
            count_occurrences_in_text(
        ...
        self.assertEqual(
            1, count_occurrences_in_text("Laboratory Floor", "'''Linguist Specialist Found Dead on Laboratory Floor'''")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Floor", "'''Linguist Specialist Found Dead on Laboratory Floor'''")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Floor", "''Linguist Specialist Found Dead on Laboratory Floor''")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Floor", "__Linguist Specialist Found Dead on Laboratory Floor__")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Floor", "'''''Linguist Specialist Found Dead on Laboratory Floor'''''")
        )
        ...
        self.assertEqual(
            1, count_occurrences_in_text("Linguist", "__Linguist Specialist Found Dead on Laboratory Floor__")
        )
        self.assertEqual(
            1, count_occurrences_in_text("Linguist", "'''''Linguist Specialist Found Dead on Laboratory Floor'''''")
        )
